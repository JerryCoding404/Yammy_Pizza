...
public class FoodOrder extends MenuItem {
	...
	//Step2. Calculate Price & Calorie with each orderItem & in total
	/* including
	...
	//Step3. after customer submit payment, clean up current data for the next food order
	
	public void CompleteOrder() throws Exception {
		//record all orderItems to the orderDetail
		...
		for(int i = 0 ; i < orderItem.itemAmount ; i++) {
			...
			SQL.orderDescription = data.orderDescription[data.customerOrderID-1];
		}
		for(int i = 0 ; i < orderItem.itemAmount ; i++) {
			data.orderIncome[data.customerOrderID-1] = orderItem.pizzaPrice[i] + orderItem.addOnPrice[i];
			SQL.orderPrice = data.orderIncome[data.customerOrderID-1];
		}
		data.customerOrderID++;
		
		//orderDetail input to database
		SQL.createTable();
		SQL.post();
